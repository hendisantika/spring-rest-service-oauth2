...
image::https://travis-ci.org/hendisantika/spring-rest-service-oauth2.svg[Build Status, link=https://travis-ci.org/hendisantika/spring-rest-service-oauth2/]
...
```sh
curl -X POST -vu clientapp:123456 http://localhost:8080/oauth/token -H "Accept: application/json" -d "password=spring&username=hendi&grant_type=password&scope=read%20write&client_secret=123456&client_id=clientapp"
```
...
```sh
curl http://localhost:8080/greeting -H "Authorization: Bearer ff16372e-38a7-4e29-88c2-1fb92897f558" | json_pp
```
